...
munitor_header "calculate_version"

GITVERSION_TAG="${GITVERSION_TAG:-5.12.0}"

# Munitor manages GitVersion config -- always write the standard SDLC-compliant config
if [[ -f "GitVersion.yml" ]]; then
  echo "WARNING: Repo contains GitVersion.yml -- Munitor manages versioning config. Repo file will be ignored."
fi
...
branches:
  ...
  hotfix:
    ...
    is-release-branch: true
GITVERSION
echo "GitVersion config written (Munitor standard SDLC)"
