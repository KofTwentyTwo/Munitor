...
render_fixture() {
  ...
  FABER_CONFIG="${fixture}" bash "${GENERATE_SCRIPT}" >/dev/null 2>&1
...
run_test "java-webapp: renders image_name" \
  "${FIXTURES_DIR}/java-webapp.faber.yml" \
  "me-health-portal"
...
run_test "java-webapp-minimal: renders java 17" \
  "${FIXTURES_DIR}/java-webapp-minimal.faber.yml" \
  "java_version: \"17\""
...
run_test "node-api: renders image_name" \
  "${FIXTURES_DIR}/node-api.faber.yml" \
  "mes-assembly-server"
...
run_test "node-api-minimal: renders node 22" \
  "${FIXTURES_DIR}/node-api-minimal.faber.yml" \
  "node_version: \"22\""
...
run_test "node-api-services: includes npm_auth when private_registry true" \
  "${FIXTURES_DIR}/node-api-services.faber.yml" \
  "npm_auth: true"
...
run_test "node-api-partial: includes test_commands" \
  "${FIXTURES_DIR}/node-api-partial.faber.yml" \
  "test_commands:"
...
run_test "sdk-distribution: renders pipeline type" \
  "${FIXTURES_DIR}/sdk-distribution.faber.yml" \
  "sdk-release"
...
run_negative_test "java-webapp-no-cd: no update-cd-repo without cd.repo" \
  "${FIXTURES_DIR}/java-webapp-no-cd.faber.yml" \
  "update-cd-repo"
...
run_context_test "java-webapp-portal: health_port set to 8000" \
  "${FIXTURES_DIR}/java-webapp-portal.faber.yml" \
  "name: docker-build-push" \
  "context:" \
  'health_port: "8000"'
...
run_context_test "java-webapp-sast-warn: sast_scan has fail_on_findings false" \
  "${FIXTURES_DIR}/java-webapp-sast-warn.faber.yml" \
  "name: sast-scan" \
  "filters:" \
...
run_context_test "terraform-sast: sast_scan has fail_on_findings true" \
  "${FIXTURES_DIR}/terraform-sast.faber.yml" \
  "name: sast-scan" \
  "filters:" \
  'fail_on_findings: "true"'
...
run_test "terraform: has release-candidate workflow" \
  "${FIXTURES_DIR}/terraform.faber.yml" \
  "release-candidate:"
